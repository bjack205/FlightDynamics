...
function delta_t = TECS_T(Et_tilde, init, P)
    ...
    % Derivative
    ud = P.kd_E*differentiator;
    
    % Integral
    ui = P.ki_E * integrator;
    ...
    % Anti-windup
    if P.ki_E ~= 0
        integrator = integrator + (P.Ts/P.ki_E)*(delta_t - delta_t_unsat);
    end
...
function theta_c = TECS_theta(Ed_tilde, init, P)
    ...
    % Derivative
    ud = P.kd_B*differentiator;
    
    % Integral
    ui = P.ki_B * integrator;
    ...
    % Anti-windup
    if P.ki_B ~= 0
        integrator = integrator + (P.Ts/P.ki_B)*(theta_c - theta_c_unsat);
    end
